...
function getScore(data, sessionData) {
    ...
    try {
        ...
        score = objective.getScore(
            data.fake_player ? data.fake_player : sessionData.player
        );
    } catch {}
...
formatter.addFunction("rank_joiner", (callData, sessionData) => {
    return getRanks
        .getRanksFromNameAndTagList(
            sessionData.player ? sessionData.player.name : "SYSTEM",
            sessionData.player
                ? sessionData.player.getTags()
                : ["rank:You cant use ranks in a non-player environment"]
        )
