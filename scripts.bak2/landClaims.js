...
export function createLandClaim(player) {
    ...
    if (
        keyval.has(
            `claim:${chunkCoordinates.x},${chunkCoordinates.z}:${player.dimension.id}`
        )
    )
        return false;
    keyval.set(
...
function getMaxY(x, z) {
    for (
        let i = world.getDimension("overworld").heightRange.max;
        i > world.getDimension("overworld").heightRange.min;
        i--
    ) {
        if (
            !world.getDimension("overworld").getBlock({ x: x, y: i, z: z })
                .isAir
        )
            return i;
    }
...
function loop() {
    ...
    for (const player of world.getPlayers()) {
        ...
        if (
            `${chunk.x},${chunk.z}` !=
            `${previousChunkLocation.x},${previousChunkLocation.z}`
        ) {
            if (
                keyval.has(
                    `claim:${chunk.x},${chunk.z}:${player.dimension.id}`
                ) &&
                keyval.has(
                    `claim:${previousChunkLocation.x},${previousChunkLocation.z}:${player.dimension.id}`
                )
            ) {
                let oldKeyval = keyval.get(
                    `claim:${previousChunkLocation.x},${previousChunkLocation.z}:${player.dimension.id}`
                );
                let newKeyval = keyval.get(
                    `claim:${chunk.x},${chunk.z}:${player.dimension.id}`
                );
            ...
            if (
                keyval.has(`claim:${chunk.x},${chunk.z}:${player.dimension.id}`)
            ) {
                let newKeyval = keyval.get(
                    `claim:${chunk.x},${chunk.z}:${player.dimension.id}`
                );
        ...
        if (actionbar) {
            let text =
                actionbar.type == "NEW_CLAIM"
                    ? `§e${
                          playerStorage.getPlayerByID(actionbar.player)
                              ? playerStorage.getPlayerByID(actionbar.player)
                                    .name
                              : "Unknown"
                      }'s §2claim`
