...
uiManager.addUI(config.uiNames.Events.EventsAdd, "A", (player, data = {}) => {
    if (data.id || data.type) {
        ...
        if (type == "KILL") {
            ...
            modalForm.textField(
                "Comment§c* - Display name for this command",
                "/say hi",
                data.comment ? data.comment : undefined
            );
            modalForm.textField(
                "Command - Command to run on the player who was killed",
                "/say hi",
                data.command ? data.command : undefined
            );
            modalForm.textField(
                "Self Command - Command to run on the player who killed another player",
                "/say hi",
                data.commandSelf ? data.commandSelf : undefined
            );
...
uiManager.addUI(
    config.uiNames.Events.EventsEdit,
    "Events Edit",
    (player, id) => {
        let form = new ActionForm();
...
uiManager.addUI(config.uiNames.Events.EventsRoot, "Events Root", (player) => {
    ...
    for (const event of events.eventsDb.data) {
        form.button(
            `§b${event.data.comment}\n§7${
                event.data.type == "KILL" ? "Kill Event" : "Death Event"
            }`,
            null,
            (player) => {
                uiManager.open(
