    ...
    (player, clanID) => {
        ...
        if (clan) {
            ...
            for (const member of OpenClanAPI.getClanMembers(clanID)) {
                ...
                form.button(
                    `${member == clan.data.owner ? "§6" : "§2"}${player.name}`,
                    null,
                    (player) => {
                        if (playerStorage.getID(player) == clan.data.owner) {
                            ...
                            form2.button(
                                `§eTransfer Ownership\n§7Give ownership to this user`,
                                null,
                                (player) => {
                                    uiManager.open(
                                        player,
                                        config.uiNames.Basic.Confirmation,
                                        "Are you sure you want to transfer ownership to this player?",
                                        () => {
                                            OpenClanAPI.transferOwnership(
                                                clan.id,
