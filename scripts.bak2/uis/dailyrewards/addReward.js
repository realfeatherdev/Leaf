...
uiManager.addUI(
    config.uiNames.DailyRewards.AddReward,
    "Edit Reward",
    (player, data = {}, index = -1, rewardsKey = "REWARDS") => {
        ...
        if (!data.type) {
            ...
            form.button(
                `§eCurrency Reward\n§7Give money to player`,
                icons.resolve(`leaf/image-479`),
                (player) => {
                    data.type = "currency";
                    uiManager.open(
                        player,
            ...
            if (data.type == "command") {
                ...
                modal.textField(
                    "Command§c*",
                    "/give @s apple 64",
                    data.command ? data.command : undefined
                );
                modal.textField(
                    "Message on claim§c*",
                    "You have received 64 apples!",
                ...
                modal.show(player, false, (player, response) => {
                    ...
                    if (index >= 0) {
                        ...
                        uiManager.open(
                            player,
                            config.uiNames.DailyRewards.Root,
                            rewardsKey
                        );
                    } else {
                        dailyRewards.createReward(rewardsKey, data.type, {
                            command: response.formValues[0],
                            message: response.formValues[1],
                        });
                        uiManager.open(
                            player,
                            config.uiNames.DailyRewards.Root,
                    ...
                    (player, num, currency) => {
                        if (!isNaN(num)) {
                            if (index >= 0) {
                                ...
                                dailyRewards.createReward(
                                    rewardsKey,
                                    data.type,
                                    { currency: currency, amount: num }
                                );
                                uiManager.open(
                                    player,
                                    config.uiNames.DailyRewards.Root,
