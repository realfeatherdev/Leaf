...
uiManager.addUI(config.uiNames.DailyRewards.Claim, "Claim Reward", (player) => {
    ...
    if (dailyRewards.hasReward(player)) {
        ...
        form.button(
            "§eClaim\n§7Claim the reward",
            icons.resolve("Packs/Asteroid/winPING"),
            (player) => {
        ...
        form.body(
            `You can claim another reward §e${moment(
                Date.now() + dailyRewards.millisecondsUntilNextDay()
            ).fromNow()}`
        );
        form.button(
            `§cBack\n§7Go back`,
...
uiManager.addUI(
    config.uiNames.DailyRewards.ClaimWeekly,
    "Claim Reward",
    (player) => {
        let form = new ActionForm();
        ...
        if (dailyRewards.hasRewardWeekly(player)) {
            ...
            form.body(
                `You can claim another reward §e${moment(
                    Date.now() + dailyRewards.millisecondsUntilNextWeek()
                ).fromNow()}`
            );
            form.button(
                `§cBack\n§7Go back`,
...
uiManager.addUI(
    config.uiNames.DailyRewards.ClaimMonthly,
    "Claim Reward",
    (player) => {
        let form = new ActionForm();
        ...
        if (dailyRewards.hasRewardMonthly(player)) {
            ...
            form.body(
                `You can claim another reward §e${moment(
                    Date.now() + dailyRewards.millisecondsUntilNextMonth()
                ).fromNow()}`
            );
            form.button(
                `§cBack\n§7Go back`,
