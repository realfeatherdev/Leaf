...
let db = prismarineDb
    .customStorage("WhatList", SegmentedStoragePrismarine)
    .keyval("WhatList");
let theme = {
    category: "§8",
    description: "§7",
...
commandManager.addCommand(
    "what",
    {
        ...
        format: "!what <post | view> [text]",
    },
    ({ msg, args }) => {
        if (!db.get("WhatList")) db.set("WhatList", "[]");
        let whatList = JSON.parse(
            db.get("WhatList") ? db.get("WhatList") : "[]"
        );
        if (args.length) {
            ...
            if (args[0] == "post") {
                ...
                let text = [];
                text.push(
                    `${theme.category}<-=- ${theme.command}What List ${theme.category}-=->`
                );
                for (const post of whatList) {
                    text.push(
                        `${theme.command}${post.s} ${theme.description}${post.t}§r`
                    );
                }
                text.push(``);
