...
var hasStringTagBug =
    ...
    isIE11 =
        typeof Map !== "undefined" && hasObjectTag(/* @__PURE__ */ new Map());
...
var EntityPersistentStorage = class {
    ...
    load(table) {
        let val = ``;
        try {
            val = __privateGet(this, _entity).getDynamicProperty(
                `prismarine:${table}`
            );
        } catch {
            val = ``;
    ...
    save(table, data) {
        system.run(() => {
            __privateGet(this, _entity).setDynamicProperty(
                `prismarine:${table}`,
                JSON.stringify(data)
            );
        });
...
var Economy = class {
    constructor(table) {
        ...
        __privateGet(this, _table)
            .waitLoad()
            .then(() => {
                if (
                    !__privateGet(this, _table).findFirst({
                        type: "CURRENCY",
                        default: true,
                    })
                ) {
    ...
    getCurrencies() {
        let array = [];
        for (const doc2 of __privateGet(this, _table).findDocuments({
            type: "CURRENCY",
        })) {
    ...
    getCurrency(scoreboard = "default") {
        if (scoreboard == "default") {
            let doc3 = __privateGet(this, _table).findFirst({
                type: "CURRENCY",
                default: true,
            });
        ...
        __privateGet(this, _form)
            ...
            .then((res) => {
                ...
                if (
                    ...
                    typeof this.callbacks[res.selection] === "function"
                ) {
...
var ModalForm = class {
    ...
    textField(id2, label, placeholder, defaultValue) {
        __privateGet(this, _form2).textField(
            label,
            placeholder,
        ...
        if (id2) {
            __privateGet(this, _controlMap).set(
                id2,
                __privateGet(this, _current)
            );
        }
    }
    slider(id2, label, min2, max2, step, defaultValue) {
        __privateGet(this, _form2).slider(
            label,
            min2,
            max2,
            step,
    ...
    dropdown(id2, label, options, defaultValue) {
        __privateGet(this, _form2).dropdown(
            label,
            options,
    ...
    toggle(id2, label, defaultValue) {
        __privateGet(this, _form2).toggle(
            label,
            ...
            .then((res) => {
                ...
                if (responseFn)
                    responseFn.call(
                        {
                            get: (id2) => {
                                if (!__privateGet(this, _controlMap).has(id2))
                                    return null;
                                return res.formValues[
                                    __privateGet(this, _controlMap).get(id2)
                                ];
                            },
                        },
                        player,
...
var _storage,
    ...
    _reservedKeyVal,
    _reservedKeyVals,
    _reservedEconomyTable,
    _PrismarineDB_instances,
    getStorage_fn,
    _variants;
...
var PrismarineDBTable = class {
    ...
    loadFolders() {
        return new Promise((resolve) => {
            system.run(() => {
                this.folders = __privateGet(this, _storage).load(
                    `${this.table}~folders`
                );
    ...
    saveFolders() {
        __privateGet(this, _storage).save(
            `${this.table}~folders`,
            this.folders
        );
    }
    loadTrash() {
        return new Promise((resolve) => {
            system.run(() => {
                this.trash = __privateGet(this, _storage).load(
                    `${this.table}~trash`
                );
                resolve(this.trash);
            });
    ...
    saveTrash(trashedData = null) {
        system.run(() => {
            ...
            try {
                if (
                    trashedData &&
                    __privateGet(this, _storage).trashable &&
                    __privateGet(this, _storage).name &&
                    stores[__privateGet(this, _storage).name]
                ) {
    ...
    getFolderDocuments(name) {
        ...
        for (const document of folder.documentIDs) {
            ...
            if (!doc2) {
                folder.documentIDs = folder.documentIDs.filter(
                    (_2) => _2 != document
                );
                this.saveFolders();
            }
    ...
    addDocumentToFolder(name, id2) {
        ...
        if (folder < 0) return;
        if (
            this.folders[folder].documentIDs.includes(id2) ||
            !this.getByID(id2)
        )
            return;
    ...
    matchesQuery(query, data) {
        if (typeof query === "string" && typeof data === "string")
            return query == data;
        if (typeof query === "object" && typeof data === "string") {
            if (query.lengthGreaterThan)
                return data.length > query.lengthGreaterThan;
        ...
        if (typeof query === "object" && typeof data === "object") {
            if (data.type && data.type == "Date") {
                if (query.after)
                    return new Date(data.timestamp).getTime() > query.after;
                if (query.before)
                    return new Date(data.timestamp).getTime() < query.before;
                return false;
            } else {
                let matchQueryRecursive = function (query2) {
                    for (const key in query2) {
                        if (typeof query2[key] !== "object") {
                            if (!data[key] || data[key] != query2[key])
                                unsuccessfulRuns++;
    ...
    updateFirstDocumentByQuery(query, data) {
        ...
        if (typeof data === "object")
            return this.overwriteDataByID(
                doc2.id,
                MergeRecursive(doc2.data, data)
            );
        else return this.overwriteDataByID(doc2.id, data);
    }
    ...
    getTrashedDocuments() {
        for (const trash of this.trash) {
            if (Date.now() >= trash.expirationDate)
                this.deleteTrashedDocumentByID(trash.id);
        }
        return this.trash;
    }
...
keyval_fn = function (id2) {
    const get2 = (key, defaultValue = null) => {
        ...
        let val = doc2.data.__keyval_data[key]
            ? doc2.data.__keyval_data[key].data
            : null;
    ...
    const set = (key, val) => {
        ...
        let doc2 = this.getByID(id2);
        if (!doc2) return;
        let currentValue = doc2.data.__keyval_data[key]
            ? doc2.data.__keyval_data[key].data
            : null;
        let newValue = {};
...
var EventHandler = class {
    ...
    attachToDb(table) {
        table.onUpdate(() => {
            if (!this.killed) this.emit("$DB_UPDATE", table);
        });
        this.attachedToDb = true;
    }
    emitToHandler(namespace, event, ...args) {
        if (namespace.startsWith("+PRISM:"))
            throw new Error("Cant emit to reserved handlers");
        if (eventHandlers[namespace] && !eventHandlers[namespace].killed)
            eventHandlers[namespace].emit(
    ...
    onDbUpdate(fn) {
        if (!this.attachedToDb)
            throw new Error(
                "You cant check for DB updates before attaching a DB"
            );
        this.events.push({
            name: "$DB_UPDATE",
            handle: fn,
...
var ExtensionRegistry = class {
    constructor(name) {
        ...
        this.onInsert = () => {};
        this.onDelete = () => {};
        __privateMethod(this, _ExtensionRegistry_instances, initialize_fn).call(
            this
        );
    }
    onInsert(fn) {
...
var FunctionStore = class {
    ...
    call(name, ...args) {
        if (!__privateGet(this, _funcs).has(name))
            throw new Error(
                `Error: No function named "${name}" in ${__privateGet(
                    this,
                    _groupName
                )}""`
            );
...
var FunctionStoreMain = class {
    ...
    getStore(name) {
        if (__privateGet(this, _stores)[name])
...
var PositionalDB = class {
    ...
    getPosition(obj) {
        ...
        let get2 = (key) => {
            ...
            try {
                data = JSON.parse(
                    world4.getDynamicProperty(
                        `prismarine_positionaldb_${__privateGet(
                            this,
                            _table2
                        )}:${vec3toString(obj)}`
                    )
...
class SegmentedStoragePrismarine {
    load(table) {
        let segmentCount = 0;
        try {
            segmentCount = world2.getDynamicProperty(
        ...
        for (let i = 0; i < segmentCount; i++) {
            let valToAppend = ``;
            try {
                valToAppend = world2.getDynamicProperty(
...
var PermissionSystem = class {
    constructor() {
        ...
        __privateSet(this, _db, new PrismarineDBTable("+PRISM:permsv6", new SegmentedStoragePrismarine()));
      // __privateGet(this, _db).clear();
        __privateGet(this, _db).waitLoad().then(()=>{
    ...
    getDefaultPermissions() {
        return __privateGet(this, _defaultPermissions);
    }
    createRole(tag) {
...
var PrismarineDB = class {
    constructor() {
        ...
        __privateSet(
            this,
            _reservedTable,
            new PrismarineDBTable("+PRISM:main", new WorldPersistentStorage())
        );
        __privateSet(
            this,
            _reservedEconomyTable,
            new PrismarineDBTable(
                "+PRISM:economy",
                new WorldPersistentStorage()
            )
        );
        __privateSet(
            this,
            _reservedKeyVal,
            __privateGet(this, _reservedTable).keyval("+PRISM:reserved")
        );
    ...
    async keyval(name) {
        if (name.startsWith("+PRISM:"))
            throw new Error(
                "Keyval names starting with '+PRISM:' are reserved"
            );
        if (__privateGet(this, _reservedKeyVals)[name])
            return __privateGet(this, _reservedKeyVals)[name];
        __privateGet(this, _reservedKeyVals)[name] = await __privateGet(
            this,
            _reservedTable
        ).keyval(name);
        return __privateGet(this, _reservedKeyVals)[name];
    ...
    table(name) {
        if (name.startsWith("+PRISM:"))
            throw new Error(
                "Database names starting with '+PRISM:' are reserved"
            );
    ...
    entityTable(name, entity) {
        ...
        let session = new PrismarineDBTable(
            name,
            new EntityPersistentStorage(entity)
        );
        sessions[generateUUID()] = session;
        return session;
    }
    ...
    scoreboardStorage(name) {
        ...
        let session = new PrismarineDBTable(
            name,
            new ScoreboardPersistentStorage()
        );
        sessions[generateUUID()] = session;
        return session;
    }
    ...
    getEventHandler(namespace) {
        if (
            ...
            (eventHandlers[namespace] && eventHandlers[namespace].killed)
        ) {
    ...
    convertBetweenStorageTypes(table, config1, config2) {
        let db1 = new PrismarineDBTable(
            table,
            new (__privateMethod(this, _PrismarineDB_instances, getStorage_fn))(
                config1.storage,
                ...(config1.params ? config1.params : [])
            )
        );
        db1.load();
        let db2 = new PrismarineDBTable(
            table,
            new (__privateMethod(this, _PrismarineDB_instances, getStorage_fn))(
                config2.storage,
...
var ColorAPI = class {
    ...
    keyToColorCode(key) {
        ...
        if (key >= this.getColorCodes().length)
            return this.getColorCodes()[this.getColorCodes().length - 1];
    ...
    getVariants(code) {
        if (!this.isValidColorCode(code))
            return __privateGet(this, _variants).get("\xA70");
...
var Player = class {
    ...
    getTagsStartingWithPrefix(player, prefix, removePrefix = false) {
        ...
        if (tags && tags.length) {
            return removePrefix
                ? tags.map((tag) => tag.substring(prefix.length))
                : tags;
        } else {
            return [];
        }
