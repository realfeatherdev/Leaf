var pjXML = (function () {
    ...
    Lexer.escapeXML = function (s) {
        ...
        return s.replace(/([<>&'"])/g, function (m, p1) {
            return "&" + Lexer.escapeMap[p1] + ";";
        });
    };
    ...
    Lexer.prototype.readName = function () {
        ...
        while (
            (ch = this.peek()) &&
            !(Lexer.isSpace(ch) || Lexer.isMarkup(ch))
        ) {
            name += this.read();
        }
            ...
            case "string":
                {
                    if (
                        this.content.length &&
                        typeof this.content[this.content.length - 1] == "string"
                    ) {
    ...
    Node.prototype.appendAll = function (o) {
        ...
        if (
            (o === null && o === undefined) ||
            typeof o[Symbol.iterator] !== "fuction"
        ) {
            this.content.push(o);
                    ...
                    default:
                        {
                            ...
                            while (
                                ...
                                var an = lex.readName();
                                if (lex.consumeString("=")) {
                                    en.attributes[an] = lex.replaceEntities(
                                        lex.readQuotedString()
                                    );
                                } else {
                        ...
                        for (var name in this.attributes) {
                            if (this.attributes.hasOwnProperty(name)) {
                                ...
                                if (typeof this.attributes[name] === "string") {
                                    s +=
                                        '="' +
                                        Lexer.escapeXML(this.attributes[name]) +
                                        '"';
                                }
                            }
                        }
