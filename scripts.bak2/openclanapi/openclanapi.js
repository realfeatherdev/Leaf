...
class OpenClanAPI {
    ...
    createClan(player, clanName) {
        if (this.db.findFirst({ type: "CLAN", name: clanName }))
            throw new Error("Clan name is already taken");
        if (clanName.length < 2)
            throw new Error("Clan name must be minimum 2 characters");
        if (clanName.length > 10)
            throw new Error("Clan name can be only be 10 characters at most");
    ...
    getClan(player) {
        let playerID = playerStorage.getID(player);
        return this.keyval.has(playerID) && this.keyval.get(playerID) != -1
            ? this.db.getByID(this.keyval.get(playerID))
            : null;
    }
}
