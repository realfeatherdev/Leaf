...
class WarpAPI {
    ...
    getWarp(name) {
        return uiBuilder.db.findFirst({ type: 12, name });
    }
    getWarps() {
        return this.db.data;
    }
    tpToWarp(player, name, system = false) {
        if (!(player instanceof Player)) return;
        let warp = uiBuilder.db.findFirst({ type: 12, name });
        if (!warp) return false;
        if(configAPI.getProperty("AFKSystem") && configAPI.getProperty("AFKWarp") && name == configAPI.getProperty("AFKWarp") && !system) return false;
        if (
