...
uiManager.addUI(config.uiNames.SidebarEditorTrash, "Trash", (player) => {
    ...
    form.title("Trash");
    form.body(
        "Here are some deleted sidebars. You can recover them here, but they will be deleted in 30 days if they are not recovered"
    );
    ...
    for (const sidebar of trashedSidebars) {
        form.button(
            `§n${sidebar.data._name}\n§r§7Deletes on ${moment(
            ...
            (player) => {
                uiManager.open(
                    player,
                    config.uiNames.SidebarEditorTrashEdit,
                    sidebar.id
                );
            }
        );
