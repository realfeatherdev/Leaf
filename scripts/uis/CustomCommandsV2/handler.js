...
function runActions(player, actions, extravars = {}, formatcfg = {}) {
    for(const action of actions) {
        if(typeof action === "string" || action.type == 0) {
            let action2 = formatStr(typeof action === "string" ? action : action.action, formatcfg.swap ? formatcfg.player2 : player, extravars, formatcfg);
            try {
            ...
            if(normalForm.playerIsAllowed(player, action.condition)) {
                runActions(player, action.actions)
